...
public class FormacionServiceImpl implements FormacionService {
	...
	} // De esta manera, de cara a a las pruebas podemos decidir inyectarles otros
		// objetos, suplantadores.
	...
	// NO ES TRANSACCIONAL, ES EL METODO DONDE SE REALIZA UNA OPERACION BASE DE
	// DATOS.
	@Transactional
	@Override
	public boolean matricularAlumno(String usuario, int idCurso) {
		...
		// Clase para encapsular datos, en vez de toma Alumno, es toma Option <Alumno>
		// para informar de si ha habido un resultado
		// Evitar que devuelva null para evitar NullPointer, te lo devuelve encapsulado
		// en un Optional. Te devuelve vacio NO NULL.
		// isEmpty() isPresent() si hay resultado. metodo get() te devuelve el contenido
		// encapsulado T.
		// orElse() hace get() y dato alternativo si no hay resultado.

		/*
		 * Alumno alumno = alumnosDao.findById(usuario).orElse(null); Curso curso =
		 * cursosDao.findById(idCurso).orElse(null); //Logica de negocio. if
		 * (alumno!=null && curso!=null) { alumno.getCursos().add(curso);
		 * //alumnosDao.update(alumno); alumnosDao.save(alumno); return true; } return
		 * false;
		 */
	...
	/**
	 * Guarda/Inserta un alumno en la BBDD pero sin repetir usuario
	 * 
	 * @param recibe el alumno a insertar
	 */
	@Override
	public boolean altaAlumno(Alumno a) {
		Optional<Alumno> alumno = alumnosDao.findById(a.getUsuario());
		if (alumno.isEmpty()) {
			alumnosDao.save(a);
			return true;
		}
	...
	/**
	 * Guarda/Inserta un curso en la BBDD pero no debe permitir repetir nombre de
	 * curso
	 * 
	 * @param recibe el curso a insertar
	 */
	@Override
	...
	@Override
	public List<Curso> listaCursosNoMatriculados(String usuario) {
